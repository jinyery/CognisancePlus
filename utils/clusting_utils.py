...
class CoarseLeadingForest:
    ...
    def get_noises(self, detailed_paths, min_size=3, start_depth=15, num_layer=1, density_percentile=25):
        a = 0
        b = 0
        c = 0
        noises = list()
        ...
        for path in detailed_paths:
            ...
            if len(path) >= start_depth:
                remaining = len(path) - start_depth + 1
        ...
        final_noises = list(filter(lambda x:self.density[x]<density_threshold, noises))
        c = len(final_noises)
        print(f"a:{a}, b:{b}, c:{c}")
        return final_noises
