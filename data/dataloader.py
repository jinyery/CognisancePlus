...
from .DT_Animal10NLT import Animal10NLT
from .DT_Food101NLT import Food101NLT
...
def get_loader(config, phase, testset, logger):
    if config['dataset']['name'] in ('MSCOCO-LT', 'MSCOCO-BL'):
        ...
        split = Food101N(phase=phase,
                             ...
                             output_path=config['output_dir'], 
                             logger=logger)
    elif config['dataset']['name'] == "Animal10NLT":
        split = Animal10NLT(phase=phase,
                             data_path=config['dataset']['data_path'], 
                             rgb_mean=config['dataset']['rgb_mean'],
                             rgb_std=config['dataset']['rgb_std'],
                             rand_aug = config['dataset']['rand_aug'],
                             output_path=config['output_dir'], 
                             logger=logger)
    elif config['dataset']['name'] == "Food101NLT":
        split = Food101NLT(phase=phase,
                             data_path=config['dataset']['data_path'], 
                             anno_path=config['dataset']['anno_path'],
                             rgb_mean=config['dataset']['rgb_mean'],
                             rgb_std=config['dataset']['rgb_std'],
