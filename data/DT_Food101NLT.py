...
class Food101NLT(data.Dataset):
    def __init__(
        ...
        super(Food101NLT, self).__init__()
    ...
    def load_img_info(self):
        ...
        if self.phase == "train":
            anno_path = os.path.join(self.anno_path, "verified_train_selected2.tsv")
        else:
            anno_path = os.path.join(self.anno_path, "verified_val.tsv")
        data = pd.read_csv(anno_path, sep="\t", header=0)
        for index, row in data.iterrows():
            # if self.phase != "train" and row["verification_label"]==0:
            #     continue
