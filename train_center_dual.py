...
class train_center_dual:
    def __init__(self, args, config, logger, eval=False):
        ...
        if self.algorithm_opt["triplet_loss"]:
            self.loss_center = CenterTripletLoss(
        ...
        if (
            ...
            and "update_steps" not in self.algorithm_opt
        ):
            self.algorithm_opt["update_steps"] = 1
    ...
    def run(self):
        ...
        for epoch in range(num_epoch):
            ...
            for step, (
                ...
                if self.algorithm_opt["triplet_loss"]:
                    loss_ct = (
                        self.loss_center(features, predictions, labels) * center_weight
                    )
                else:
            ...
            if (
                self.algorithm_opt["always_update"]
                and epoch % self.algorithm_opt["update_steps"] == 0
                ...
                elif self.algorithm_opt["env_type"] in (
                    "inter",
                    "intra",
                    "inter_intra",
                ):
    ...
    def update_env_by_uniform(self, env1_loader, env2_loader, total_image):
        ...
        self.logger.info(
            "Env1 and Env2 have size {} and {}".format(
                all_scores.sum().item(), all_scores.sum().item()
            )
        )
    ...
    def update_env_by_correct(self, env1_loader, env2_loader, total_image):
        ...
        correct_index = (
            self.all_ind[self.all_lgt.max(1)[1] == self.all_lab].cpu().tolist()
        )
        ...
        self.logger.info(
            "Env1 and Env2 have size {} and {}".format(
                env1_score.sum().item(), env2_score.sum().item()
            )
        )
    ...
    def update_env_by_score(self, env1_loader, env2_loader, total_image):
        ...
        if self.algorithm_opt["env_type"] in ("inter", "inter_intra"):
            inter_weight = self.generate_inter_weight(
                all_scores, total_image, tg_scale=self.algorithm_opt["sample_scale"]
            )
            env2_score = env2_score * inter_weight
        if self.algorithm_opt["env_type"] in ("intra", "inter_intra"):
            intra_weight = self.generate_intra_weight(
                cat_socres, total_image, tg_scale=self.algorithm_opt["sample_scale"]
            )
    ...
    def generate_inter_weight(self, all_scores, total_image, tg_scale=4.0):
        ...
        head_mean = (
            torch.topk(inter_weight, k=int(total_image * 0.8), largest=False)[0]
            .mean()
            .item()
        )
        tail_mean = (
            torch.topk(inter_weight, k=int(total_image * 0.2), largest=True)[0]
            .mean()
            .item()
        )
        ...
        self.logger.info(
            "Inter Score Original Head (80) Tail (20) Scale is {}".format(scale)
        )
        self.logger.info(
            "Inter Score Target   Head (80) Tail (20) Scale is {}".format(tg_scale)
        )
    ...
    def generate_intra_weight(self, cat_socres, total_image, tg_scale=4.0):
        ...
        for cat, cat_items in cat_socres.items():
            ...
            head_mean = (
                torch.topk(cat_scores, k=int(cat_size * 0.8), largest=False)[0]
                .mean()
                .item()
            )
            tail_mean = (
                torch.topk(cat_scores, k=int(cat_size * 0.2), largest=True)[0]
                .mean()
                .item()
            )
            scale = tail_mean / head_mean + 1e-5
            ...
            if int(cat) == 0:
                self.logger.info(
                    "Intra Score at Cat-{} Original Head (80) Tail (20) Scale is {}".format(
                        cat, scale
                    )
                )
                self.logger.info(
                    "Intra Score at Cat-{} Target   Head (80) Tail (20) Scale is {}".format(
                        cat, tg_scale
                    )
                )
                self.logger.info(
                    "Intra Score at Cat-{} Exp Scale is {}".format(
                        cat, exp_scale.item()
                    )
                )
        ...
        self.logger.info(
            "Intra Total Score {}, which should be equal to NUM_CLASS".format(
                intra_weight.sum().item()
            )
        )
        return intra_weight
